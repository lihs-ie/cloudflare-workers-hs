...
Haskell の Servant を Cloudflare Workers (workerd) で実運用するためのライブラリ。
設計判断は `docs/adr/` の ADR-0001〜0018 (採用済み) を参照。エージェント規約の詳細・結線点・rubric は
`AGENTS.md` を参照する。

## Agent Policy

<!-- GENERATED by agent-policy-kit from ~/.claude/docs/agent-policy.md. 直接編集せず正本を直して再適用すること。 -->
...
- **placeholder stub** (`throwError err501` / `notImplemented` / `todo!()` 等) を本番に残さない。wire-first で結線を先に通す。
- 例外は `ci/allowlist.yml` に owner/reason/expires_at 付きで登録 (無期限禁止)。
- **フォーク禁止・全て自前実装** (ADR-0006)。認証は **Cloudflare Zero Trust を最大限活用** (ADR-0009)。

### 命名・略語 (project ルール)
- 略語は多数が共通認識として理解できるもの (`URL`, `URI`, `UUID`, `ULID` 等) のみ許可。独自短縮は使用禁止、識別子は省略しない。
- 識別子型名は `XXXId` ではなく `XXXIdentifier`。自身の識別子フィールドは `identifier`。`Detail`/`Info` 等の曖昧語をモデル名に使わない。
