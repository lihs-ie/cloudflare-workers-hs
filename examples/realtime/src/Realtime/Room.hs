...
server storage state connectionIdentifier =
    RoomRoutes
  ...
  where
    connectWith tag attachment _ request _
        | requestMethod request /= GET =
            pure (createResponse (Status 405) (headersFromList [("Allow", "GET")]) (ResponseBodyBytes "GET required"))
        | fmap Text.toCaseFold (headerLookup "Upgrade" (requestHeaders request)) /= Just "websocket" =
            pure (createResponse (Status 426) (headersFromList []) (ResponseBodyBytes "WebSocket upgrade required"))
        | otherwise = do
...
onMessage storage state socket payload = do
    ...
    case decoded of
        ...
        Right (Just connection) -> do
            ...
            if Bytes.length bytes > 4096
                then webSocketClose socket 1009 "Message exceeds 4096 bytes"
                else do
