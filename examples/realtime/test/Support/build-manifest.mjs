...
function artifactHashes() {
  return Object.fromEntries(["worker/realtime-worker.wasm", "worker/realtime-worker-jsffi.mjs"].map(path => [path, digest(resolve(root, path))]));
}
export function verifyBuild() {
  const proof = JSON.parse(readFileSync(resolve(root, "worker/build.json"), "utf8"));
  if (JSON.stringify(proof.sources) !== JSON.stringify(sourceHashes())) throw new Error("Realtime worker WASM is stale; run bash examples/realtime/scripts/build.sh");
  if (JSON.stringify(proof.artifacts) !== JSON.stringify(artifactHashes())) throw new Error("Realtime worker WASM/glue digest mismatch");
}
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (operation === "capture") writeFileSync(path, JSON.stringify(sourceHashes()));
    ...
    if (JSON.stringify(before) !== JSON.stringify(after)) throw new Error("Realtime worker inputs changed while compiling; rebuild");
