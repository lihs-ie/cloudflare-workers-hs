...
kvValidation kv = do
    ...
    pure $
        object
            [ "farFutureRejected" .= case future of Left (KVPutFailed _) -> True; _ -> False
...
cacheValidation = do
    ...
    pure $
        object
            [ "partialRejected" .= isLeft partial
            , "varyRejected" .= isLeft vary
            , "partialClassification" .= cacheFailure partial
            , "varyClassification" .= cacheFailure vary
            , "absent" .= isNothing absent
            , "recovered" .= isJust recovered
            , "deleted" .= deleted
...
cacheMethod = do
    ...
    pure $
        object
            [ "putRejected" .= isLeft rejected
            , "classification" .= cacheFailure rejected
            , "message" .= case rejected of Left (CachePutRejected _ message) -> message; _ -> ""
            , "strictMiss" .= isNothing strict
            , "ignoredHit" .= isJust ignored
            , "strictDelete" .= strictDelete
            , "ignoredDelete" .= ignoredDelete
            , "absent" .= isNothing absent
            ]
