        ...
        object
            [ "batch"
                .= map
                    ( \(key, result) ->
                        object
                            ["key" .= key, "value" .= textValue (kvMetadataResultValue result), "metadata" .= kvMetadataResultMetadata result]
                    )
                    (kvBulkMetadataResultValues batch)
...
storageScenario kv "formats" = do
    ...
    pure $
        object
            [ "json" .= case json of Just (KVJSONValue value) -> Just value; _ -> Nothing
...
storageScenario kv "json-metadata" = do
    ...
    pure $
        object
            [ "expiration" .= map kvListKeyExpiration (kvListResultKeys listed)
            , "listedMetadata" .= map kvListKeyMetadata (kvListResultKeys listed)
            , "value" .= jsonValue (kvMetadataResultValue single)
            , "metadata" .= kvMetadataResultMetadata single
            , "missing" .= isNothing (kvMetadataResultValue missing)
    ...
    results <-
        mapM
            ( \(name, policy) -> do
                ...
                pure $ object ["policy" .= directive, "hit" .= isJust found]
            )
            policies
...
storageScenario _ "regional-cache-write" = do
    ...
    cachePut
        ...
        ( createResponse
            (Status 200)
            (headersFromList [("Cache-Control", "public, max-age=120")])
            (ResponseBodyBytes "regional guide")
        )
