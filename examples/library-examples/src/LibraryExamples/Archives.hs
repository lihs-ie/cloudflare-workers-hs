...
import Cloudflare.Workers.Headers (headerInsert, headersFromList)
...
import Servant.API (Capture, NamedRoutes, Raw, (:>))
...
archiveServer bucket secret =
    ArchiveRoutes
        { encrypted =
            handle
                "encrypted"
                ( case secret >>= mkR2SsecKey . encodeUtf8 . revealSecret of
    ...
    handle policy encryption identifier segments request _
        ...
        | otherwise = case encryption of
            ...
            Right key -> do
                ...
                pure (fromRight (failure 502 "Archive storage failed") result)
    dispatch objectKey key request = case requestMethod request of
        POST -> do
            ...
            pure
                ( createResponse
                    (Status 201)
                    (headerInsert "Content-Type" "application/json" safeHeaders)
                    ( ResponseBodyLazyBytes
                        ( encode
                            ( object
                                [ "size" .= r2ObjectMetaSize meta
