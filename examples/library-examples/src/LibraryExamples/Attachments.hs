...
attachmentHandler bucket secret [identifier] request
    | validIdentifier identifier = case encryption of
        ...
        Right (namespace, key) -> do
            ...
            pure (either (const (failure 502 "Attachment storage failed")) id result)
  where
        ...
        PUT -> case requestBodyReader request of
            ...
            Just readBody -> do
                ...
                case bytes of
                    ...
                    Right body -> do
                        stored <- r2Put bucket objectKey (R2PutBytes (Lazy.toStrict body)) r2PutDefaultOptions
                            { r2ExtendedPutHttpMetadata = r2HttpMetadataDefault
                                { r2HttpMetadataContentType = Just (maybe "application/octet-stream" id (headerLookup "Content-Type" (requestHeaders request)))
        ...
        GET -> do
            ...
            case found of
                ...
                R2GetSuccess object -> do
                    ...
                    pure (createResponse (Status 200)
...
validIdentifier value = not (Text.null value) && Text.length value <= 128
