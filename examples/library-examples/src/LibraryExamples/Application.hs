...
import LibraryExamples.API (API, GuideAPI, GuideRoutes (..), Routes (..))
...
import LibraryExamples.SocketExamples (SocketEndpoints (..), socketStructuredGreeting, socketTLSGreeting)
import LibraryExamples.Storage (storageScenario)
import Network.HTTP.Types.Status qualified as HTTPStatus
import Servant.Client.Core (BaseUrl (..), ClientError (..), ResponseF (..), Scheme (..), clientIn)
...
server database validator producer jobNamespace attachmentSecret clientBase kv service connector bucket endpoints configurationSettings =
    Routes
...
settings kv = do
    kvPut
        kv
        "display:guide"
        (KVPutText "Download exports from the management API")
    ...
    pure $
        object
            [ "value" .= textValue (kvMetadataResultValue value)
            , "metadata" .= kvMetadataResultMetadata value
            , "batch" .= map (\(key, item) -> object ["key" .= key, "value" .= textValue item]) (kvBulkResultValues batch)
            , "keys" .= map kvListKeyName (kvListResultKeys listing)
            , "complete" .= kvListResultListComplete listing
            ]
  where
...
mapped action =
    mapExceptionsToServerError
        ( \failure -> case failure of
            ...
            Jobs.JobConflict -> ServerError 409 "Conflict" [] Nothing
        )
        (liftIO action)
