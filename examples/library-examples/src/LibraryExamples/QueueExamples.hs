...
queueExampleServer producer kv =
    QueueRoutes
        { metricsDiagnostic = liftIO $ do
            ...
            pure $ case outcome of
                ...
                Left (QueueMetricsFailed reason)
                    | "unavailable in this runtime" `Text.isInfixOf` reason ->
                        object ["status" .= ("unsupported" :: Text), "metrics" .= (Nothing :: Maybe Value)]
                Left _ -> object ["status" .= ("failed" :: Text), "metrics" .= (Nothing :: Maybe Value)]
        , advisoryCheck = \name -> do
            when (Text.null name || Text.length name > 80) $ throwError Errors.err400
  ...
  where
    send transport job = do
        ...
        when (Text.null name || Text.length name > 80 || Text.any (\c -> not (isAsciiLower c || isAsciiUpper c || isDigit c || c == '-' || c == '_')) name || Text.null (payload job)) $ throwError Errors.err400
