...
pageLimit proposed = do
  let size = maybe 100 id proposed
...
handleListURLs after proposed = do
  ...
  rows <- liftIO $ query env.database "SELECT * FROM urls WHERE identifier>? ORDER BY identifier LIMIT ?" [D1Text (maybe "" id after),D1Integer (toInteger (size+1))]
  values <- liftIO $ traverse rowURL (take size rows)
  let next = if length rows > size then case reverse values of
...
handleStats startMaybe endMaybe after proposed = do
  ...
  size <- pageLimit proposed
  rows <- liftIO $ query env.database "SELECT url,day,count FROM daily_clicks WHERE day>=? AND day<=? AND (url || '/' || day)>? ORDER BY url,day LIMIT ?" [D1Text (T.pack (show start)),D1Text (T.pack (show end)),D1Text (maybe "" id after),D1Integer (toInteger (size+1))]
  entries <- liftIO $ traverse (\row -> do
    code <- textColumn "url" row
    day <- textColumn "day" row
    count <- integerColumn "count" row
    pure (object ["url" .= code,"day" .= day,"count" .= count],code <> "/" <> day)) (take size rows)
  let next = if length rows > size then case reverse entries of
