  ...
  where
    listEvents cursor = liftIO $ do
        ...
        rows <-
            query
                (database env)
                "SELECT identifier,url,occurred_at,status,last_error FROM failed_events WHERE identifier>? AND expires_at>? ORDER BY identifier LIMIT 100"
        ...
        traverse
            ( \row -> do
                ...
                pure (object ["identifier" .= identifier, "url" .= url, "occurredAt" .= occurred, "status" .= state])
            )
            rows
