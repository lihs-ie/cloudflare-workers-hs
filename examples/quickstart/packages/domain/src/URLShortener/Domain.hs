...
module URLShortener.Domain
    ( CreateURL (..), EditURL (..), ShortURL (..), ClickEvent (..)
    , DateRange, mkDateRange, rangeStart, rangeEnd
    , validateDestination, validateCreateURL, validateEditURL
    , isRedirectable, eventWithinWindow
    , idempotencyExpiresAt, eventExpiresAt, exportExpiresAt
    ) where
...
data ShortURL = ShortURL
    { identifier :: Text, destination :: Text, createdAt :: UTCTime
    , expiresAt :: Maybe UTCTime, deletedAt :: Maybe UTCTime, version :: Int }
    deriving (Eq, Show, Generic, FromJSON, ToJSON)
...
validateDestination value = case parseURI (T.unpack value) of
  ...
  where
    validPort "" = True
    validPort (':' : digits) = not (null digits) && all isDigit digits
        && length digits <= 5 && (read digits :: Int) > 0 && (read digits :: Int) <= 65535
    validPort _ = False
    allowedHost host
    ...
    publicHost host = T.length host <= 253 && length labels >= 2
        && all validLabel labels && T.any isAsciiLower (last labels)
      ...
      where labels = T.splitOn "." host
    validLabel label = not (T.null label) && T.length label <= 63
...
isRedirectable now (ShortURL _ _ created expiry deleted _) =
    created <= now && deleted == Nothing && maybe True (now <) expiry
...
publicIPv4 address = any matches ["192.0.0.9/32", "192.0.0.10/32"]
    || not (any matches
        [ "0.0.0.0/8", "10.0.0.0/8", "100.64.0.0/10", "127.0.0.0/8"
        , "169.254.0.0/16", "172.16.0.0/12", "192.0.0.0/24", "192.0.2.0/24"
        , "192.88.99.0/24", "192.168.0.0/16", "198.18.0.0/15"
        , "198.51.100.0/24", "203.0.113.0/24", "224.0.0.0/3" ])
  where matches = inPrefix 32 parseIPv4 address

publicIPv6 :: Integer -> Bool
publicIPv6 address = any matches
    [ "64:ff9b::/96", "2001:1::1/128", "2001:1::2/128", "2001:1::3/128"
    , "2001:3::/32", "2001:4:112::/48", "2001:20::/28", "2001:30::/28" ]
    || (matches "2000::/3" && not (any matches
        ["2001::/23", "2001:db8::/32", "2002::/16", "3fff::/20"]))
  where matches = inPrefix 128 parseIPv6 address
...
parseIPv4 raw = do
    ...
    if length pieces /= 4 then Nothing else do
  ...
  where
    octet value
        ...
        | not (T.all (\c -> c >= '0' && c <= '9') value) = Nothing
        | otherwise = do
    ...
    groups <- case T.splitOn "::" raw of
        ...
        [left, right] -> do
            ...
            if missing > 0 && not (T.any (== '.') left)
                then Just (before <> replicate missing 0 <> after) else Nothing
        _ -> Nothing
...
inPrefix width parseAddress address prefix = case T.splitOn "/" prefix of
    [host, bits] -> case (parseAddress host, readMaybe (T.unpack bits)) of
        (Just network, Just size) | size >= 0 && size <= width ->
