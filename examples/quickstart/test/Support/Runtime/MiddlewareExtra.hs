...
import Cloudflare.Workers.Middleware (formatRequestIdentifierUUIDv4, withStructuredLogging, withStructuredLoggingUsing)
...
middlewareExtraProbe request context = do
    ...
    let sink record = do
            ...
            when (count == 4) $ putMVar completed ()
        handler received (_ :: BindingEnv '[] '[] '[]) execution = do
            ...
            outcome <-
                try @IOException $
                    logging
                        (\_ _ _ -> throwIO (userError "middleware-known-failure"))
                        received
                        ()
                        execution
            ...
            _ <-
                logging
                    (\_ _ _ -> pure (createResponse (Status 204) (headersFromList []) (ResponseBodyBytes "")))
                    received
                    ()
                    execution
            registrationReceipts <-
                if requestPath received == "/middleware-native"
                    then forM [LogDebug, LogWarn] $ \level ->
                        deferredSinkExceptErrors
                            execution
                            (emitLog defaultLoggerConfig)
