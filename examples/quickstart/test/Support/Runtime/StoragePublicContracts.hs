...
exceptionContract name failures = do
    outcomes <- mapM (\failure -> do
        outcome <- try (throwIO failure)
        pure (case outcome of Left actual -> actual == failure && displayException actual == show failure; Right (_ :: ()) -> False)) failures
...
storagePublicContractsProbe raw = do
    ...
    result <- case mode of
        ...
        "exceptions" -> object . (\xs -> ["exceptions" .= xs]) <$> sequence
        ...
        "ordering" -> pure $ let
            ...
            in object ["contained" .= all inRange [lower,middle,upper], "strict" .= (lower < middle && upper > middle && upper >= middle), "compare" .= (compare lower upper == LT && compare upper lower == GT && compare middle middle == EQ), "endpoints" .= (min lower upper == lower && max lower upper == upper), "deduplicated" .= (nub [lower,middle,lower,upper] == [lower,middle,upper]), "contracts" .= [contract "WorkflowIdentifier" [lower,middle,upper]]]
...
requiredFieldContract name expected encoded = object
    [ "name" .= name
