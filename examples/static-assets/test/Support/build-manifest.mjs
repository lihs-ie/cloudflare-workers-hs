...
const inputs = ["tsconfig.json", "../../scripts/testing/typecheck.mjs", "worker/wasm.d.ts", "../../pnpm-lock.yaml", "../../pnpm-workspace.yaml", "../../package.json", "package.json", "app", "src", "public", "scripts", "test", "worker/index.ts", "worker/runtime.ts", "static-assets-worker.cabal", "../../cabal.project", "../../cabal-wasm.project", "../../cabal-wasm-coverage.project", "wrangler.jsonc",
...
function artifactHashes() {
  return Object.fromEntries(["worker/static-assets-worker.wasm", "worker/static-assets-worker-jsffi.mjs"].map(path => [path, digest(resolve(root, path))]));
}
export function verifyBuild() {
  const proof = JSON.parse(readFileSync(resolve(root, "worker/build.json"), "utf8"));
  if (JSON.stringify(proof.sources) !== JSON.stringify(sourceHashes())) throw new Error("Static assets worker WASM is stale; run bash examples/static-assets/scripts/build.sh");
  if (JSON.stringify(proof.artifacts) !== JSON.stringify(artifactHashes())) throw new Error("Static assets worker WASM/glue digest mismatch");
}
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (operation === "capture") writeFileSync(path, JSON.stringify(sourceHashes()));
    ...
    if (JSON.stringify(before) !== JSON.stringify(after)) throw new Error("Static assets worker inputs changed while compiling; rebuild");
