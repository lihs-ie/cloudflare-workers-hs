...
spec = do
    ...
    it "rejects wrong container types, absent nested fields and trailing timestamp junk" $ do
        map
            (\payload -> isLeft (eitherDecode payload :: Either String ApprovalRequest))
            ["[]", "null", "{}", "{\"message\":1,\"behavior\":\"approve\"}", "{\"message\":\"ok\"}"]
            `shouldBe` replicate 5 True
        map
            (\payload -> isLeft (eitherDecode payload :: Either String CreateRequest))
            ["[]", "{\"identifier\":1,\"parameters\":{}}", "{\"identifier\":\"x\",\"parameters\":null}"]
            `shouldBe` replicate 3 True
        map
            (\payload -> isLeft (eitherDecode payload :: Either String Approval))
            ["false", "{}", "{\"approved\":null}"]
            `shouldBe` replicate 3 True
        mapM_
            ( \stamp -> case decodeAt (String stamp) of
                Error _ -> pure ()
