...
server workflow database = Routes
    { health = pure (object ["status" .= ("ok" :: Text)])
    , create = \input -> do
        when (Text.null (identifier input) || Text.null (message (parameters input)) || behavior (parameters input) `notElem` ["normal", "retry", "fail"])
    ...
    , createGenerated = \input -> do
        when (Text.null (message input) || behavior input `notElem` ["normal", "retry", "fail"])
    ...
    , control = \identifier' operation -> case lookup operation
    ...
    , audit = \identifier' -> liftIO (auditValue database identifier')
    }
  where
...
recordAttempt database instance' stepName attempt = void $ Query.d1Execute database
    (Query.D1Statement "INSERT INTO attempts(instance, step, attempt) VALUES (?, ?, ?)"
...
runApproval database event step = do
    let instance' = unWorkflowIdentifier (workflowEventInstance event)
        ...
        options = defaultWorkflowStepOptions
            {workflowRetryLimit=2, workflowRetryDelay=WorkflowMilliseconds 100, workflowStepTimeout=WorkflowMilliseconds 10000}
    ...
    committed <- workflowStepDo step "commit-request" options $ \context -> do
        ...
        void $ Query.d1Execute database (Query.D1Statement "INSERT INTO effects(instance, message) VALUES (?, ?) ON CONFLICT(instance) DO NOTHING"
...
auditValue database instance' = do
    attempts <- Query.d1Query database
    ...
    effects <- Query.d1QueryFirst database
