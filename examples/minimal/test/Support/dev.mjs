...
export async function startDev(options = {}) {
  ...
  const state = await mkdtemp(path.join(tmpdir(), "minimal-worker-"));
  const artifact = path.resolve(example, "../../artifacts/testing", `minimal-${new Date().toISOString().replaceAll(/[:.]/g, "-")}`);
