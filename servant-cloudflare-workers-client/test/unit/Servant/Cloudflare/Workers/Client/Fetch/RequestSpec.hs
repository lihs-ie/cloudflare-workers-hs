...
spec = describe "Fetch request conversion" $ do
    it "preserves encoded values and separates query flags from empty values" $ do
        let request =
                appendToQueryString
                    "q"
                    [QN (encodeQueryParamValue ("a&b=#% 日本" :: Text.Text))]
                    $ appendToQueryString "empty" [QN ""]
                    $ appendToQueryString "flag" [] defaultRequest
    ...
    it "generated safe query values survive conversion" $ property $ do
        ...
        let request =
                appendToQueryString
                    "q"
                    [QN (encodeQueryParamValue value)]
                    defaultRequest
    ...
    it "escapes query names and preserves repeated keys in request order" $ do
        let request =
                appendToQueryString "a&b" [QN "second"] $
                    appendToQueryString "a&b" [QN "first"] defaultRequest
        buildFetchTargetURL (BaseUrl Https "example.com" 443 "") request
