...
module Cloudflare.Workers.Binding.Workflow
    ( Workflow (..), WorkflowIdentifier (..), WorkflowInstance, workflowInstanceIdentifier
    , WorkflowState (..), WorkflowStatus (..), WorkflowFailure (..), WorkflowError (..)
    , WorkflowStep, workflowStepFromJSVal, WorkflowStepContext (..)
    , WorkflowDuration (..), WorkflowBackoff (..), WorkflowStepOptions (..), defaultWorkflowStepOptions
    , WorkflowNonRetryableError (..), WorkflowReceivedEvent (..)
    , workflowCreate, workflowGet, workflowStatus, workflowSendEvent
    , workflowPause, workflowResume, workflowRestart, workflowTerminate
    , workflowStepDo, workflowSleep, workflowSleepUntil, workflowWaitForEvent
    ) where
...
import Control.Exception (Exception, SomeException, SomeAsyncException, bracket, mask_, displayException, fromException, throwIO, try)
...
data WorkflowStepOptions = WorkflowStepOptions
    { workflowRetryLimit :: Int, workflowRetryDelay :: WorkflowDuration
    , workflowRetryBackoff :: WorkflowBackoff, workflowStepTimeout :: WorkflowDuration
...
data WorkflowStepContext = WorkflowStepContext
    { workflowStepName :: Text, workflowStepCount :: Int, workflowStepAttempt :: Int
    } deriving stock (Show, Eq)
...
data WorkflowState = WorkflowQueued | WorkflowRunning | WorkflowPaused | WorkflowErrored
    | WorkflowTerminated | WorkflowComplete | WorkflowWaiting | WorkflowWaitingForPause | WorkflowUnknown Text
    deriving stock (Show, Eq)
...
data WorkflowStatus output = WorkflowStatus
    { workflowState :: WorkflowState, workflowOutput :: Maybe output, workflowFailure :: Maybe WorkflowFailure
    } deriving stock (Show, Eq)
instance FromJSON output => FromJSON (WorkflowStatus output) where
    parseJSON = withObject "WorkflowStatus" $ \o -> do
        state <- o .: "status"
        WorkflowStatus (case state of
            "queued" -> WorkflowQueued; "running" -> WorkflowRunning; "paused" -> WorkflowPaused
            "errored" -> WorkflowErrored; "terminated" -> WorkflowTerminated; "complete" -> WorkflowComplete
            "waiting" -> WorkflowWaiting; "waitingForPause" -> WorkflowWaitingForPause; other -> WorkflowUnknown other)
            <$> o .:? "output" <*> o .:? "error"
data WorkflowReceivedEvent payload = WorkflowReceivedEvent
    { workflowReceivedPayload :: payload, workflowReceivedType :: Text, workflowReceivedTimestamp :: Text
    } deriving stock (Show, Eq)
...
workflowStatus instance' = jsWorkflowStatus (workflowInstanceValue instance') >>= unwrap "status" (jsValToText >=> parseJSONText "status")
  where (>=>) first second value = first value >>= second
...
workflowStepDo (WorkflowStep step) name options action = do
    ...
    config <- textToJSVal (encodeJSONText (object
...
workflowWaitForEvent (WorkflowStep step) name eventType duration = do
    ...
    jsWorkflowWaitForEvent step rawName options >>= unwrapStep "waitForEvent" (\raw -> jsValToText raw >>= parseJSONText "waitForEvent")
