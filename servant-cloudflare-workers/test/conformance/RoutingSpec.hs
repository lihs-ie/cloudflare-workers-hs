...
import Hedgehog (annotateShow, assert, evalIO, forAll, property)
...
spec :: Spec
spec = do
    describe "fixed reference cases" $
        mapM_
            ( \input -> it (Text.unpack (caseName input)) $ do
    ...
    it "includes request and both observations in mismatch diagnostics" $ do
        ...
        case result of
            Left err ->
                mapM_
                    (\fragment -> (fragment `isInfixOf` displayException err) `shouldBe` True)
    ...
    it "inspects strict buffered adapter output and rejects native streams" $ do
        let response = HTTP.createResponse (HTTP.Status 200) (Headers.headersFromList [])
