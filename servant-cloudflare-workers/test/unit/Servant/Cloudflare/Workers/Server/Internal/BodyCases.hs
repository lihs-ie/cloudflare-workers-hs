...
spec :: Spec
spec = do
  describe "JSON body" $ mapM_ (\(body, code) -> it (show body) $ do
    response <- serveWithContext (Proxy @(ReqBody '[JSON] Text :> Post '[JSON] Text)) EmptyContext pure
      request{requestMethodField=POST,requestHeaders=headersFromList [("Content-Type","application/json")],requestBodyReaderField=Just (\_ -> pure (Right body))} context ()
  ...
  it "rejects incompatible body type before consuming body" $ do
    ...
    response <- serveWithContext (Proxy @(ReqBody '[JSON] Text :> Post '[JSON] Text)) EmptyContext pure
      request{requestMethodField=POST,requestHeaders=headersFromList [("Content-Type","text/plain")],requestBodyReaderField=Just (\_ -> modifyIORef' calls (+1) >> pure (Right "hello"))} context ()
    responseStatus response `shouldBe` Status 415
    readIORef calls `shouldReturn` 0
  it "propagates body limit failure as 413" $ do
    limits <- newIORef []
    response <- serveWithContext (Proxy @(ReqBody '[JSON] Text :> Post '[JSON] Text)) EmptyContext pure
      request{requestMethodField=POST,requestHeaders=headersFromList [("Content-Type","application/json")],requestBodyReaderField=Just (\limit -> modifyIORef' limits (<>[limit]) >> pure (Left ReadableStreamExceededByteLimit))} context ()
  ...
  it "treats an absent body reader as an empty body" $ do
    response <- serveWithContext (Proxy @(ReqBody '[PlainText] Text :> Post '[PlainText] Text)) EmptyContext pure
      request{requestMethodField=POST,requestHeaders=headersFromList [("Content-Type","text/plain;charset=utf-8")]} context ()
