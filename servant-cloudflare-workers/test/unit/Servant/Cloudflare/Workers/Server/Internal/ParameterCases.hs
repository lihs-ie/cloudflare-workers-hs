...
spec = do
    describe "QueryParam" $
        mapM_
            ( \(path, code, body) -> it (show path) $ do
    ...
    describe "Header" $
        mapM_
            ( \(headers, code, body) -> it (show headers) $ do
    ...
    it "WithNamedContext dispatches the nested API" $ do
        response <-
            serveWithContext
                (Proxy @(WithNamedContext "nested" '[Int] (Get '[JSON] Int)))
                (NamedContext @"nested" ((42 :: Int) :. EmptyContext) :. EmptyContext)
                (pure 42)
                request
                context
                ()
