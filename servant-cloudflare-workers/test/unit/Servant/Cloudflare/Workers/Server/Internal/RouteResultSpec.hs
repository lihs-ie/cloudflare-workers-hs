...
spec = do
  ...
  it "does not execute an IO continuation after either failure" $ mapM_ (\failure -> do
    ref <- newIORef (0::Int)
    ...
    readIORef ref `shouldReturn` 0) [Fail err404, FailFatal err400]
  it "lifts IO and binds successful transformer values" $
    runRouteResultT (liftIO (pure (41::Int)) >>= (pure . (+1))) `shouldReturn` Route 42
