...
import Cloudflare.Workers.HTTP (Request, Response, Status (..), ResponseBody (..), createResponse, responseStatus, requestMethod)
...
runRouterInternals request context = sequence
    [ check "static-root-markers" $ do
        mapM_ (\segments -> routeStatus (leafRouter (const success)) segments >>= assertEqual 200) [[], [""]]
    , check "capture-choice" $ do
        let router = choice (CaptureRouter [] (leafRouter (const (failure err404))))
    ...
    , check "capture-missing" $ do
        mapM_ (\segments -> routeStatus (CaptureRouter [] (leafRouter (const success))) segments >>= assertEqual 404) [[], [""]]
    ...
    , check "heterogeneous-choice" $ do
        let router = choice (RawRouter (const (failure err404)))
    ...
    , check "failure-priority" $ do
        mapM_ (\(low, high) -> do
          ...
          assertEqual False (worseHTTPCode high low)) [(404,405),(405,401),(401,415),(415,406),(406,500),(500,400)]
    ...
    , check "route-result-short-circuit" $ do
        ...
        assertEqual (Route (42 :: Int)) (Route 41 >>= pure . (+1))
        assertEqual (Route (42 :: Int)) (pure (+1) <*> Route 41)
    ...
    , check "delayed-map-preserves-failure" $ do
        ...
        mapM_ (\failed -> runDelayed
          (error "mapping a failed server must not run" <$> emptyDelayed failed) () request
          >>= assertEqual failed) [Fail err400, FailFatal err401 :: RouteResult Int]
    ...
    , check "request-extraction" $ do
        ...
        runDelayed (passToServer (emptyDelayed (FailFatal err400 :: RouteResult (Int -> Int)))
          (\_ -> error "failed server must not extract request")) () request >>= assertEqual (FailFatal err400)
    ]
  where
...
runDelayedFailures request = sequence
  ...
  where
    ...
    scenario fatal stop = do
      ...
      let step name = do
            ...
            if name == stop
              then if fatal then delayedFailFatal err400 else delayedFail err400
              else pure ()
          delayed = Delayed (const (step "capture")) (step "method") (step "auth") (step "accept")
            (step "content") (step "params") (step "headers") (const (step "body"))
...
runForwardingContracts request = sequence
  ...
  where
    ...
    base = Delayed (pure . (+ 1)) (pure ()) (pure 14) (pure ())
      (pure 5) (pure 12) (pure 13) (pure . (* 3))
      (\capture params headers auth body req -> Route
